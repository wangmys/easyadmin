define(["jquery", "easy-admin", "vue"], function ($, ea, Vue) {
    ...
    table = layui.table,
    url = {
        save_url:ea.url('system.skc.Shoeconfig/saveConfig'),
        save_skc_config_url:ea.url('system.skc.Shoeconfig/saveSkcConfig'),
        del_url:ea.url('/system.skc.Shoeconfig/delConfig'),
        index_url:ea.url('/system.skc.Shoeconfig/index')
    }
    ...
    var Controller = {
        ...
        //裤台陈列标准
        shoe_num:function (){
            var shoe_num_head = $("#shoe_num_head").val();
            var shoe_num_head = JSON.parse(shoe_num_head);
            var html = '';
            var that = this;
            shoe_num_head.forEach(function (i,value) {
                html += '<td class="field_'+ value +'_'+ i +'"><input type="text" name="' + i + '[]" lay-verify="required" value="" placeholder="请输入" class="layui-input"></td>'
           })
            $('.add_shoe_num').on('click', function(){
               var element = $([
                   '<tr>',
                       ,html,
                       '<td class="handler">',
                ...
                element.find('.get').on('click', function(){
                    ...
                    var shoe_num = element.find('input[name="shoe_num[]"]').val();
                    var skc_zt = element.find('input[name="skc_zt[]"]').val();
                    var skc_xx = element.find('input[name="skc_xx[]"]').val();
                    var skc_ydx = element.find('input[name="skc_ydx[]"]').val();
                    var skc_lx = element.find('input[name="skc_lx[]"]').val();
               ...
               $('#form-shoe_num-config tbody').append(element);
           });

            // 获取渲染后端渲染的数据,重新绑定事件
            $.each($('.skc_shoe_num-select'), function (key, element) {
        ...
        //SKC价格配置
        shoeskc_config:function (){
            var that = this;

            // 获取渲染后端渲染的数据,重新绑定事件
            $.each($('.shoeskc_config-select'), function (key, element) {

                var gender = $(element).find('.skc_shoe_nostore')[0];
                var data = JSON.parse($(element).attr('lay-data'));
                // console.log(data);
                var data = data.skc_shoe_nostore;
                ...
                $(element).find('.get_shoeskc_config').on('click', function(){
                    var _url = url.save_skc_config_url;
                    var shoe_price = $(element).find('input[name="shoe_price"]').val();
                    var sign_id = $(element).find('input[name="sign_id"]').val();
                    var skc_shoe_nostore = $(element).find('.skc_shoe_nostore .label-content').attr('title');

                    var _data = {
                        shoe_price:shoe_price,
                        sign_id:sign_id,
                        skc_shoe_nostore:skc_shoe_nostore
                    }
                    // console.log(_data);
                    // 保存配置
