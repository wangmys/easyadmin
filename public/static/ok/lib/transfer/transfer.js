layui.define('table',function (exports) {
    ...
    var $ = layui.$
        ,table = layui.table
        ,MOD_NAME = 'transfox',LEFT_TABLE = 'left-table-',RIGHT_TABLE = 'right-table-',LEFT_BTN = 'left-btn-',RIGHT_BTN = 'right-btn-',FILTER= 'test'
        ,DISABLED = 'layui-btn-disabled',BTN = 'button',BTN_STLY='btn',MD5='layui-col-md4',MD2='layui-col-md1'
        ,transfox = {
            index:layui.transfox?(layui.transfox+1000):0
            ,idData:[] //ID池
            ,data:[] //原始数据
            ,options:[]
        ...
        ,Class = function (options) {
            var that = this
            that.index = transfox.index?transfox.index:++transfox.index
            that.config = options
            transfox.options = options
    ...
    Class.prototype.render = function () {
        ...
        transfox.idData.push(that.index)
        transfox.data = [];
        transfox.data.push({id:LEFT_TABLE+that.index,data:(options.data[0]?options.data[0]:[])})
        transfox.data.push({id:RIGHT_TABLE+that.index,data:(options.data[1]?options.data[1]:[])})
        table.render(d1_c)
        table.render(d2_c)
        that.move()
    };
    ...
    function datas (type) {
        ...
        var options = transfox.options
        options.data =datasChecked(_d);
        transfox.render(options)
    }
    ...
    table.on('checkbox('+FILTER+')', function(obj){
        var idData = transfox.idData,lenght = idData.length;
        if(lenght > 0){
    ...
    transfox.render = function (options) {
        var inst = new Class(options)
        return thisRate.call(inst)
    }
    exports(MOD_NAME,transfox)
})
