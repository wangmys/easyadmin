...
/**
 * @ControllerAnnotation(title="断码率_夏")
 * 改成 top50 了
 */
class DuanmalvSummer extends BaseController
{
    // 接收筛选参数
    public $params = [];
    // 数据库
    ...
    public function __construct()
    {
        ...
        // 冬
        $config = $this->db_easyA->table('cwl_duanmalv_config')->where(['status' => 1, 'id' => 4])->find();
        $this->config = $config;
        $this->top = $config['top'];
    }
    ...
    -- 								END  AS 当前零售价,
        ...
        if ($select) {
            ...
            $this->db_easyA->execute('TRUNCATE cwl_duanmalv_retail_summer;');

            $chunk_list = array_chunk($select, 500);

            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_retail_summer')->strict(false)->insertAll($val);
            }
            ...
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_summer where 店铺云仓='南昌云仓' and 商品代码 in ({$noGoodsNo_nc})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_summer where 店铺云仓='广州云仓' and 商品代码 in ({$noGoodsNo_gz})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_summer where 店铺云仓='武汉云仓' and 商品代码 in ({$noGoodsNo_wh})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_summer where 店铺云仓='长沙云仓' and 商品代码 in ({$noGoodsNo_cs})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_summer where 店铺云仓='贵阳云仓' and 商品代码 in ({$noGoodsNo_gy})
            ");

            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => "cwl_duanmalv_retail_summer first 更新成功，数量：{$count}！"
            ]);
        }
    }
    ...
    public function retail_second() {
        // 更新零售价
        $sql0 = "update cwl_duanmalv_retail_summer as r left join sjp_goods as g on r.`商品代码` = g.货号 
        set r.零售价 = g.零售价
        where r.零售价 is null";
        ...
        // 计算折率
        $sql1 = "
            UPDATE cwl_duanmalv_retail_summer 
        ...
        if ($select) {
            ...
            // 删除 需要计算排名的
            $this->db_easyA->table('cwl_duanmalv_retail_summer')->where([
                // ['折率', '>=', 0.9]
                ['折率', '>=', $find_config['折率']]
            ])->delete();
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_retail_summer second 更新成功！'
            ]);
        } else {
            ...
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_retail_summer 排名执行失败！'
            ]);
        }
    }
    ...
    public function sk_first()
    {
        ...
        if ($select_sk) {
            ...
            $this->db_easyA->execute('TRUNCATE cwl_duanmalv_sk_summer;');
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_sk_summer')->strict(false)->insertAll($val);
                if (! $insert) {
                    $status = false;
                    break;
                }
            ...
            if ($status) {
                ...
                return json([
                    'status' => 1,
                    'msg' => 'success',
                    'content' => "cwl_duanmalv_sk_summer first 更新成功，数量：{$count}！"
                ]);
            } else {
                // $this->db_easyA->rollback();
                return json([
                    'status' => 0,
                    'msg' => 'error',
                    'content' => 'cwl_duanmalv_sk_summer first 更新失败！'
                ]);
            }
        }
    }
    ...
    public function sk_second() {
        ...
        $sql = "
            UPDATE 
                cwl_duanmalv_sk_summer AS sk
            INNER JOIN
                cwl_duanmalv_retail_summer AS dr 
            ON 
        ...
        if ($status) {
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => "cwl_duanmalv_sk_summer 店铺排名 零售价 当前零售价 更新成功，数量：{$status}！"
            ]);
        } else {
            // $this->db_easyA->rollback();
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_sk_summer 店铺排名 零售价 当前零售价 更新失败！'
            ]);
        }
    }
    ...
    public function sk_third() {
        ...
        if ($status) {
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => "cwl_duanmalv_sk_summer 标准齐码识别修订 更新成功，数量：{$status}！"
            ]);
        } else {
            // $this->db_easyA->rollback();
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_sk_summer 标准齐码识别修订 更新失败！'
            ]);
        }
    }
    ...
    public function handle_1_new() {
        $sql1_old = "
            ...
            FROM
                (SELECT sk.经营模式,
                    ...
                    ( SELECT count(店铺SKC计数 ) FROM cwl_duanmalv_sk_summer WHERE 店铺名称 = sk.店铺名称 AND 风格 = sk.风格 AND sk.一级分类=一级分类 AND sk.二级分类=二级分类 AND
                    sk.领型=领型 and 店铺SKC计数=1) AS SKC数,
                    ( SELECT sum(店铺SKC计数 ) FROM cwl_duanmalv_sk_summer WHERE 店铺名称 = sk.店铺名称 AND 风格 = sk.风格 ) AS 店铺总SKC数,
                    sum(dr.销售金额) AS 销售金额,
                    (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_summer where 店铺名称=sk.店铺名称 AND 风格=sk.风格) AS 店铺总销售金额
                    from cwl_duanmalv_sk_summer as sk
                    LEFT JOIN cwl_duanmalv_retail_summer as dr ON sk.货号 = dr.`商品代码` AND sk.`店铺名称` = dr.`店铺名称` 
                    where 
                    sk.风格 IN ('基本款', '引流款')
        ...
        FROM
            (SELECT sk.经营模式,
                ...
                ( SELECT count(店铺SKC计数 ) FROM cwl_duanmalv_sk_summer WHERE 店铺名称 = sk.店铺名称 AND 风格 = sk.风格 AND sk.一级分类=一级分类 AND sk.二级分类=二级分类 AND
                sk.领型=领型 and 店铺SKC计数=1) AS SKC数,
                ( SELECT sum(店铺SKC计数 ) FROM cwl_duanmalv_sk_summer WHERE 店铺名称 = sk.店铺名称 ) AS 店铺总SKC数,
                ( SELECT sum(店铺SKC计数 ) FROM cwl_duanmalv_sk_summer WHERE 店铺名称 = sk.店铺名称 AND 风格 = '基本款' ) AS 店铺SKC数_基本款,
                ( SELECT sum(店铺SKC计数 ) FROM cwl_duanmalv_sk_summer WHERE 店铺名称 = sk.店铺名称 AND 风格 = '引流款' ) AS 店铺SKC数_引流款,
                sum(dr.销售金额) AS 销售金额,
                (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_summer where 店铺名称=sk.店铺名称 AND 风格='基本款') AS 店铺总销售金额_基本款,	
                (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_summer where 店铺名称=sk.店铺名称 AND 风格='引流款') AS 店铺总销售金额_引流款,
                (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_summer where 店铺名称=sk.店铺名称 AND 风格 in ('基本款', '引流款')) AS 店铺总销售金额
                from cwl_duanmalv_sk_summer as sk
                LEFT JOIN cwl_duanmalv_retail_summer as dr ON sk.货号 = dr.`商品代码` AND sk.`店铺名称` = dr.`店铺名称` 
            where 1
        ...
        if ($select) {
            ...
            $this->db_easyA->execute('TRUNCATE cwl_duanmalv_handle_1_summer;');

            $chunk_list = array_chunk($select, 500);

            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $status = $this->db_easyA->table('cwl_duanmalv_handle_1_summer')->strict(false)->insertAll($val);
            }

            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_handle_1_summer 更新成功！'
            ]);
        }
    }
    ...
    public function handle_2() {
        ...
        $sql = "
            update cwl_duanmalv_handle_1_summer h
                LEFT JOIN cwl_duanmalv_sk_summer sk ON h.店铺名称 = sk.店铺名称 
                AND h.`二级分类` = sk.`二级分类` 
                AND h.风格=sk.风格
                AND h.领型 = sk.领型
                SET sk.`是否TOP60` = '是'
        ...
        // 2.没有在途，是top60考核款
        $sql2 = "
            update cwl_duanmalv_sk_summer sk
        ...
        if ($status) {
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_sk_summer 是否top60考核款 更新成功！'
            ]);
        } else {
            // $this->db_easyA->rollback();
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_sk_summer 是否top60考核款  更新失败！'
            ]);
        }
    }
    ...
    public function handle_3() {
        ...
        $sql = "
            SELECT sk.店铺名称,
                ...
                COUNT(1) AS 全部断码SKC数	
                from cwl_duanmalv_sk_summer sk 
            WHERE sk.`是否TOP60`='是'
            GROUP BY sk.`店铺名称`,sk.风格, sk.一级分类,sk.`二级分类`,sk.领型
        ";
        ...
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_sk_group_summer')->where(1)->delete();
            $this->db_easyA->table('cwl_duanmalv_sk_group_summer')->strict(false)->insertAll($select);

            $sql2 = "
                UPDATE cwl_duanmalv_handle_1_summer AS h
                LEFT JOIN cwl_duanmalv_sk_group_summer skg ON h.`店铺名称` = skg.`店铺名称` 
                AND h.风格 = skg.风格 
                AND h.一级分类 = skg.`一级分类` 
                AND h.`二级分类` = skg.`二级分类` 
                AND h.领型 = skg.领型 
            ...
            $this->db_easyA->table('cwl_duanmalv_config')->where('id=4')->strict(false)->update([
            ...
            if ($status) {
                ...
                return json([
                    'status' => 1,
                    'msg' => 'success',
                    'content' => 'cwl_duanmalv_handle_1_summer TOP断码数  全部断码数 更新成功！'
                ]);
            } else {
                // $this->db_easyA->rollback();
                return json([
                    'status' => 0,
                    'msg' => 'error',
                    'content' => 'cwl_duanmalv_handle_1_summer TOP断码数  全部断码数 更新失败！'
                ]);
            }
        } else {
            // $this->db_easyA->rollback();
    ...
    public function table6() {
        $sql = "
            ...
            FROM
                cwl_duanmalv_handle_1_summer 
            ORDER BY
                商品负责人 ASC
        ";
        $select = $this->db_easyA->query($sql);
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_table6_summer')->where(1)->delete();
            $this->db_bi->table('ww_duanmalv_table6_summer')->where(1)->delete();
            $chunk_list = array_chunk($select, 500);
            $status = true;
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $this->db_easyA->table('cwl_duanmalv_table6_summer')->strict(false)->insertAll($val);
                $this->db_bi->table('ww_duanmalv_table6_summer')->strict(false)->insertAll($val);
            }
            $this->db_easyA->table('cwl_duanmalv_config')->where('id=4')->strict(false)->update([
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_table6_summer 更新成功！'
            ]);
        } else {
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_table6_summer 更新失败！'
            ]);
        }
    }
    ...
    public function table5() {
        $sql = "
            ...
            from cwl_duanmalv_handle_1_summer as h
        ...
        $select = $this->db_easyA->query($sql);
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_table5_summer')->where(1)->delete();
            $chunk_list = array_chunk($select, 500);
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $this->db_easyA->table('cwl_duanmalv_table5_summer')->strict(false)->insertAll($val);
            }
            ...
            $sql2 = "
                ...
                from cwl_duanmalv_table5_summer as t5,
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_table5_summer 更新成功！'
            ]);
        } else {
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_table5_summer 更新失败！'
            ]);
        }
    }
    ...
    public function table4() {
        $sql = "
            SELECT
                ...
                sum(销售数量) as 销售数量
                FROM
                    cwl_duanmalv_retail_summer where `商品代码` = sk.货号
                    AND 省份 = sk.省份
                    group by 省份,商品代码
                ) AS 销售数量,
        ...
        $select = $this->db_easyA->query($sql);
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_table4_summer')->where(1)->delete();
            $chunk_list = array_chunk($select, 500);
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $this->db_easyA->table('cwl_duanmalv_table4_summer')->strict(false)->insertAll($val);
            }
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_table4_summer 更新成功！'
            ]);
        } else {
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_table4_summer 更新失败！'
            ]);
        }
    }
    ...
    public function table3() {
        ...
        $select = $this->db_easyA->query($sql);
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_table3_summer')->where(1)->delete();
            $chunk_list = array_chunk($select, 500);
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $this->db_easyA->table('cwl_duanmalv_table3_summer')->strict(false)->insertAll($val);
            }

            $sql2 = "
                UPDATE cwl_duanmalv_table3_summer AS A
                LEFT JOIN (
                    SELECT
                        风格,大类,中类,领型,
                        MAX(单款排名) AS 最后排名 
                    FROM
                        `cwl_duanmalv_table3_summer` 
                    GROUP BY
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_table3_summer 更新成功！'
            ]);
        } else {
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_table3_summer 更新失败！'
            ]);
        }
    }
    ...
    public function table1() {
        ...
        $sql = "
            SELECT 
                h1.*,
                -- (SELECT COUNT(*) FROM cwl_duanmalv_sk_summer WHERE 店铺名称=h1.店铺名称 AND `标准齐码识别修订`='断码') AS '断码数-整体',
                1 - (SELECT COUNT(*) FROM cwl_duanmalv_sk_summer WHERE 店铺名称=h1.店铺名称 AND `标准齐码识别修订`='断码' AND 风格 in ('基本款')) / `SKC数-整体` AS '齐码率-整体',
                ...
                FROM 
                    ...
                    FROM cwl_duanmalv_handle_1_summer h0 
        ...
        if ($select) {
            // 只删除当天
            $this->db_easyA->table('cwl_duanmalv_table1_1_summer')->where([
                '更新日期' => $date
            ])->delete();

            $this->db_bi->table('cwl_duanmalv_table1_1_summer')->where([
                '更新日期' => $date
            ])->delete();
            // die;
            $chunk_list = array_chunk($select, 500);
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_table1_1_summer')->strict(false)->insertAll($val);
                $insert = $this->db_binew->table('cwl_duanmalv_table1_1_summer')->strict(false)->insertAll($val);
            }
            $this->table1_1_sort();   
    ...
    protected function table1_avg() {
        ...
        $sql_old = "
            ...
            FROM
                ...
                SELECT
                    ...
                    jm.`加盟-TOP考核` 
                FROM
                    cwl_duanmalv_table1_1_summer AS zy
                    LEFT JOIN (
                    SELECT
                        云仓,商品负责人,
                        AVG( `齐码率-整体` ) AS `加盟-整体`,
        ...
        $sql = "
            SELECT
                    ...
                    date_format(now(),'%Y-%m-%d') as 更新日期 
            FROM
                    cwl_duanmalv_table1_1_summer AS hj
                    LEFT JOIN (
                        SELECT
                                云仓,商品负责人,
        ...
        $select = $this->db_easyA->query($sql);
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_table1_avg_summer')->where([
                '更新日期' => $date
            ])->delete();
            $chunk_list = array_chunk($select, 500);
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_table1_avg_summer')->strict(false)->insertAll($val);
            }
        }
    }
    ...
    protected function table1_1_sort() {
        ...
        $sql = "
            ...
            FROM
                cwl_duanmalv_table1_1_summer a,
                ( SELECT @商品负责人 := null, @rank := 0 ) T 
            WHERE a.更新日期='{$date}'
            ORDER BY
                商品负责人,
    ...
    public function table1_2() {
        ...
        if ($select) {
            // 只删除当天
            $this->db_easyA->table('cwl_duanmalv_table1_2_summer')->where([
                '更新日期' => $date
            ])->delete();
            // die;
            $chunk_list = array_chunk($select, 500);
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_table1_2_summer')->strict(false)->insertAll($val);
            }
            $this->table1_2_sort();   
            $this->db_easyA->table('cwl_duanmalv_config')->where('id=4')->strict(false)->update([
    ...
    protected function table1_2_sort() {
        ...
        $sql = "
            SELECT
                ...
                @rank := @rank + 1 AS 齐码排名 
            FROM
                cwl_duanmalv_table1_2_summer a,
                ( SELECT @rank := 0 ) T 
            WHERE a.更新日期='{$date}'
            ORDER BY
                `合计-TOP考核` DESC
    ...
    public function table1_3() {
        ...
        $sql_old = "
            ...
            SELECT
                ...
                jm.`加盟-TOP考核`
            FROM
                cwl_duanmalv_table1_1_summer t1
            LEFT JOIN (
            SELECT
                省份,
                商品负责人,
        ...
        if ($select) {
            // 只删除当天
            $this->db_easyA->table('cwl_duanmalv_table1_3_summer')->where([
                '更新日期' => $date
            ])->delete();
            // die;
            $chunk_list = array_chunk($select, 500);
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_table1_3_summer')->strict(false)->insertAll($val);
            }

            $sql_del = "
                DELETE 
                FROM
                    cwl_duanmalv_table1_3_summer 
                WHERE
                    `直营-整体` IS NULL 
                    AND `加盟-整体` IS NULL 
                    AND `合计-整体` IS NULL 
    ...
    protected function table1_3_sort() {
        ...
        $sql = "
            SELECT
                ...
                @rank := @rank + 1 AS 齐码排名 
            FROM
                cwl_duanmalv_table1_3_summer a,
                ( SELECT @rank := 0 ) T 
            WHERE a.更新日期='{$date}'
            ORDER BY
                `合计-TOP考核` DESC
