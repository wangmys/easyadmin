...
class Ldkdjd extends BaseController
{
    ...
    public function handle_zy()
    {
        ...
        $同比天['销量'] = $select_同比今年[0]['销量'] - $select_同比去年[0]['销量'];
        $同比天['单数'] = $select_同比今年[0]['单数'] - $select_同比去年[0]['单数'];
        $同比天['正品业绩'] = $select_同比今年[0]['正品业绩'] - $select_同比去年[0]['正品业绩'];
        $同比天['连带'] = $select_同比今年[0]['连带'] - $select_同比去年[0]['连带'];
        $同比天['客单'] = $select_同比今年[0]['客单'] - $select_同比去年[0]['客单'];
        $同比天['件单'] = $select_同比今年[0]['件单'] - $select_同比去年[0]['件单'];
        ...
        if (date('d', strtotime($current_today)) == 1) {
            ...
            // 同比累计
            $find_昨天_同比累计 = $this->db_easyA->table('cwl_ldkdjd_handle_zy')->where([
                ['类型', '=', '同比累计'],
                ['销售日期', '=', date('Y-m-d', strtotime('-1DAY', strtotime($current_today)))]
            ])->find();  
            ...
            $同比累计['类型'] = '同比累计';
            $同比累计['性质'] = $同比天['性质'];
            $同比累计['销量'] = $同比天['销量'] + $find_昨天_同比累计['销量'];
            $同比累计['单数'] = $同比天['单数'] + $find_昨天_同比累计['单数'];
            $同比累计['正品业绩'] = $同比天['正品业绩'] + $find_昨天_同比累计['正品业绩'];
            $同比累计['连带'] = round($同比累计['销量'] / $同比累计['单数'], 1);
            $同比累计['客单'] = round($同比累计['正品业绩'] / $同比累计['单数'], 1);
            $同比累计['件单'] = round($同比累计['正品业绩'] / $同比累计['销量'], 1);
            $同比累计['销售日期'] = $同比天['销售日期'];
    ...
    public function handle_jm()
    {
        ...
        if (date('d', strtotime($current_today)) == 1) {
            ...
            // 同比累计
            $find_昨天_同比累计 = $this->db_easyA->table('cwl_ldkdjd_handle_jm')->where([
                ['类型', '=', '同比累计'],
                ['销售日期', '=', date('Y-m-d', strtotime('-1DAY', strtotime($current_today)))]
            ])->find();  
   ...
   public function handle()
   {
       ...
       if (date('d', strtotime($current_today)) == 1) {
           ...
           // 同比累计
           $find_昨天_同比累计 = $this->db_easyA->table('cwl_ldkdjd_handle')->where([
               ['类型', '=', '同比累计'],
               ['销售日期', '=', date('Y-m-d', strtotime('-1DAY', strtotime($current_today)))]
           ])->find();  
