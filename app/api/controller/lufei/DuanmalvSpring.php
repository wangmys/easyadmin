...
class DuanmalvSpring extends BaseController
{
    ...
    -- 								END  AS 当前零售价,
        ...
        if ($select) {
            ...
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_spring where 店铺云仓='南昌云仓' and 商品代码 in ({$noGoodsNo_nc})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_spring where 店铺云仓='广州云仓' and 商品代码 in ({$noGoodsNo_gz})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_spring where 店铺云仓='武汉云仓' and 商品代码 in ({$noGoodsNo_wh})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_spring where 店铺云仓='长沙云仓' and 商品代码 in ({$noGoodsNo_cs})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_retail_spring where 店铺云仓='贵阳云仓' and 商品代码 in ({$noGoodsNo_gy})
            ");
    ...
    public function sk_first()
    {
        ...
        if ($select_sk) {
            ...
            // 删除云仓不要货号的记录
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk_spring where 云仓='南昌云仓' and 货号 in ({$noGoodsNo_nc})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk_spring where 云仓='广州云仓' and 货号 in ({$noGoodsNo_gz})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk_spring where 云仓='武汉云仓' and 货号 in ({$noGoodsNo_wh})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk_spring where 云仓='长沙云仓' and 货号 in ({$noGoodsNo_cs})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk_spring where 云仓='贵阳云仓' and 货号 in ({$noGoodsNo_gy})
            ");
