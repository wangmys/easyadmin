...
class Customitem17 extends BaseController
{
    ...
    public function zhuanyuan_yeji_new() {
        ...
        if ($select) {
            ...
            $sql_100日均需销额 = "
                update cwl_customitem17_zhuanyuan_current
                set 
                    `100%日均需销额_直营` = (`目标_直营` - `累计流水_直营`) / {$日均需到结束剩余天数},
                    `100%日均需销额_加盟` = (`目标_加盟` - `累计流水_加盟`) / {$日均需到结束剩余天数},
                    `100%日均需销额_合计` = (`目标_合计` - `累计流水_合计`) / {$日均需到结束剩余天数}
                where
                    目标月份 = '{$目标月份}'
            ";
            ...
            $sql_85日均需销额 = "
                update cwl_customitem17_zhuanyuan_current
                set 
                    `85%日均需销额_直营` = (`目标_直营` * 0.85  - `累计流水_直营`) / {$日均需到结束剩余天数},
                    `85%日均需销额_加盟` = (`目标_加盟` * 0.85 - `累计流水_加盟`)  / {$日均需到结束剩余天数},
                    `85%日均需销额_合计` = (`目标_合计` * 0.85 - `累计流水_合计`)  / {$日均需到结束剩余天数}
                where
                    目标月份 = '{$目标月份}'
            ";
            $this->db_easyA->execute($sql_85日均需销额);
    ...
    public function updateZhuanyuan_total_current() {
        ...
        $合计['100%日均需销额_直营'] = $到结束剩余天数 > 0 ? round(($合计['目标_直营'] - $合计['累计流水_直营']) / $到结束剩余天数, 2): $合计['目标_直营'] - $合计['累计流水_直营'];
        $合计['100%日均需销额_加盟'] = $到结束剩余天数 > 0 ? round(($合计['目标_加盟'] - $合计['累计流水_加盟']) / $到结束剩余天数, 2): $合计['目标_加盟'] - $合计['累计流水_加盟'];
        $合计['100%日均需销额_合计'] = $到结束剩余天数 > 0 ? round(($合计['目标_合计'] - $合计['累计流水_合计']) / $到结束剩余天数, 2): $合计['目标_合计'] - $合计['累计流水_合计'];
        $合计['85%日均需销额_直营'] = $到结束剩余天数 > 0 ? round(($合计['目标_直营'] * 0.85 - $合计['累计流水_直营']) / $到结束剩余天数, 2): $合计['目标_直营'] * 0.85 - $合计['累计流水_直营'];
        $合计['85%日均需销额_加盟'] = $到结束剩余天数 > 0 ? round(($合计['目标_加盟'] * 0.85 - $合计['累计流水_加盟']) / $到结束剩余天数, 2): $合计['目标_加盟'] * 0.85 - $合计['累计流水_加盟'];
        $合计['85%日均需销额_合计'] = $到结束剩余天数 > 0 ? round(($合计['目标_合计'] * 0.85 - $合计['累计流水_合计']) / $到结束剩余天数, 2): $合计['目标_合计'] * 0.85 - $合计['累计流水_合计'];
    ...
    public function test2() {
        ...
        $目标月份 = date('Y-m');
        echo $截止日期 = date('Y-m-d H:i:s');
        // echo $截止日期 = date('2024-01-01 00:00:00');
        echo  '<br>';
        echo $本月最后一天 = date('Y-m-t 23:59:59'); 
        // echo $本月最后一天 = date('2024-01-31 23:59:59'); 
        echo  '<br>';
        $到结束剩余天数 = $this->getDaysDiff(strtotime($截止日期), strtotime($本月最后一天)) + 1;
        $str_近七天日期 = '';
        $getBefore7 = $this->getBefore7();
        if($getBefore7) {
            $str_近七天日期 = arrToStr($getBefore7);
