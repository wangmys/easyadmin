...
/**
 * @ControllerAnnotation(title="断码率_冬")
 * 改成 top50 了
 */
class DuanmalvWinter extends BaseController
{
    // 接收筛选参数
    public $params = [];
    // 数据库
    ...
    public function __construct()
    {
        ...
        // 冬
        $config = $this->db_easyA->table('cwl_duanmalv_config')->where(['status' => 1, 'id' => 2])->find();
        $this->config = $config;
        $this->top = $config['top'];
    }
    ...
    public function retail_first() {
        ...
        if ($select) {
            ...
            $this->db_easyA->execute('TRUNCATE cwl_duanmalv_retail_winter;');
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => "cwl_duanmalv_retail_winter first 更新成功，数量：{$count}！"
            ]);
        }
    }
    ...
    public function retail_second() {
        // 更新零售价
        $sql0 = "update cwl_duanmalv_retail_winter as r left join sjp_goods as g on r.`商品代码` = g.货号 
        set r.零售价 = g.零售价
        where r.零售价 is null";
        ...
        // 计算折率
        $sql1 = "
            UPDATE cwl_duanmalv_retail_winter 
        ...
        if ($select) {
            ...
            // 删除 需要计算排名的
            $this->db_easyA->table('cwl_duanmalv_retail_winter')->where([
                // ['折率', '>=', 0.9]
                ['折率', '>=', $find_config['折率']]
            ])->delete();
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_retail_winter second 更新成功！'
            ]);
        } else {
            ...
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_retail_winter 排名执行失败！'
            ]);
        }
    }
    ...
    public function sk_first()
    {
        ...
        if ($select_sk) {
            ...
            $this->db_easyA->execute('TRUNCATE cwl_duanmalv_sk_winter;');
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $insert = $this->db_easyA->table('cwl_duanmalv_sk_winter')->strict(false)->insertAll($val);
                if (! $insert) {
                    $status = false;
                    break;
                }
            ...
            if ($status) {
                ...
                return json([
                    'status' => 1,
                    'msg' => 'success',
                    'content' => "cwl_duanmalv_sk_winter first 更新成功，数量：{$count}！"
                ]);
            } else {
                // $this->db_easyA->rollback();
                return json([
                    'status' => 0,
                    'msg' => 'error',
                    'content' => 'cwl_duanmalv_sk_winter first 更新失败！'
                ]);
            }
        }
    }
    ...
    public function sk_second() {
        ...
        $sql = "
            ...
            INNER JOIN
                cwl_duanmalv_retail_winter AS dr 
            ON 
        ...
        if ($status) {
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => "cwl_duanmalv_sk_winter 店铺排名 零售价 当前零售价 更新成功，数量：{$status}！"
            ]);
        } else {
            // $this->db_easyA->rollback();
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_sk_winter 店铺排名 零售价 当前零售价 更新失败！'
            ]);
        }
    }
    ...
    public function sk_third() {
        ...
        if ($status) {
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => "cwl_duanmalv_sk_winter 标准齐码识别修订 更新成功，数量：{$status}！"
            ]);
        } else {
            // $this->db_easyA->rollback();
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_sk_winter 标准齐码识别修订 更新失败！'
            ]);
        }
    }
    ...
    public function handle_1_new() {
        ...
        FROM
            (SELECT sk.经营模式,
                ...
                sum(dr.销售金额) AS 销售金额,
                (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_winter where 店铺名称=sk.店铺名称 AND 风格='基本款') AS 店铺总销售金额_基本款,	
                (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_winter where 店铺名称=sk.店铺名称 AND 风格='引流款') AS 店铺总销售金额_引流款,
                (select sum(IFNULL(销售金额, 0)) from cwl_duanmalv_retail_winter where 店铺名称=sk.店铺名称 AND 风格 in ('基本款', '引流款')) AS 店铺总销售金额
                from cwl_duanmalv_sk_winter as sk
                LEFT JOIN cwl_duanmalv_retail_winter as dr ON sk.货号 = dr.`商品代码` AND sk.`店铺名称` = dr.`店铺名称` 
            where 1
        ...
        if ($select) {
            ...
            $this->db_easyA->execute('TRUNCATE cwl_duanmalv_handle_1_winter;');

            $chunk_list = array_chunk($select, 500);

            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $status = $this->db_easyA->table('cwl_duanmalv_handle_1_winter')->strict(false)->insertAll($val);
            }

            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_handle_1_winter 更新成功！'
            ]);
        }
    }
    ...
    public function handle_2() {
        ...
        $sql = "
            update cwl_duanmalv_handle_1_winter h
        ...
        // 2.没有在途，是top60考核款
        $sql2 = "
            update cwl_duanmalv_sk_winter sk
        ...
        if ($status) {
            ...
            return json([
                'status' => 1,
                'msg' => 'success',
                'content' => 'cwl_duanmalv_sk_winter 是否top60考核款 更新成功！'
            ]);
        } else {
            // $this->db_easyA->rollback();
            return json([
                'status' => 0,
                'msg' => 'error',
                'content' => 'cwl_duanmalv_sk_winter 是否top60考核款  更新失败！'
            ]);
        }
    }
    ...
    public function handle_3() {
        ...
        if ($select) {
            $this->db_easyA->table('cwl_duanmalv_sk_group_winter')->where(1)->delete();
            $this->db_easyA->table('cwl_duanmalv_sk_group_winter')->strict(false)->insertAll($select);

            $sql2 = "
                UPDATE cwl_duanmalv_handle_1_winter AS h
                LEFT JOIN cwl_duanmalv_sk_group_winter skg ON h.`店铺名称` = skg.`店铺名称` 
                AND h.风格 = skg.风格 
                AND h.一级分类 = skg.`一级分类` 
                AND h.`二级分类` = skg.`二级分类` 
                AND h.领型 = skg.领型 
            ...
            if ($status) {
                ...
                return json([
                    'status' => 1,
                    'msg' => 'success',
                    'content' => 'cwl_duanmalv_handle_1_winter TOP断码数  全部断码数 更新成功！'
                ]);
            } else {
                // $this->db_easyA->rollback();
                return json([
                    'status' => 0,
                    'msg' => 'error',
                    'content' => 'cwl_duanmalv_handle_1_winter TOP断码数  全部断码数 更新失败！'
                ]);
            }
        } else {
            // $this->db_easyA->rollback();
    ...
    public function table1() {
        ...
        if ($select) {
            ...
            $this->db_bi->table('cwl_duanmalv_table1_1_winter')->where([
                '更新日期' => $date
            ])->delete();
            // die;
            $chunk_list = array_chunk($select, 500);
