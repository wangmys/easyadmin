...
class Duanmalv extends BaseController
{
    ...
    public function sk_first()
    {
        ...
        if ($select_sk) {
            ...
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk where 云仓='南昌云仓' and 货号 in ({$noGoodsNo_nc})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk where 云仓='广州云仓' and 货号 in ({$noGoodsNo_gz})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk where 云仓='武汉云仓' and 货号 in ({$noGoodsNo_wh})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk where 云仓='长沙云仓' and 货号 in ({$noGoodsNo_cs})
            ");
            $this->db_easyA->execute("
                DELETE FROM cwl_duanmalv_sk where 云仓='贵阳云仓' and 货号 in ({$noGoodsNo_gy})
            ");
    ...
    public function table6() {
        ...
        if ($select) {
            ...
            $this->db_bi->table('ww_duanmalv_table6_autumn')->where(1)->delete();
            $chunk_list = array_chunk($select, 500);
            $status = true;
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $this->db_easyA->table('cwl_duanmalv_table6')->strict(false)->insertAll($val);
                $this->db_bi->table('ww_duanmalv_table6_autumn')->strict(false)->insertAll($val);
            }
            $this->db_easyA->table('cwl_duanmalv_config')->where('id=1')->strict(false)->update([
