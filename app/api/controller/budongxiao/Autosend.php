...
class Autosend extends BaseController
{
    ...
    // 区域不动销同省同二级分类数量统计
    public function areaFzrNumHandle() {
        $select_config = $this->db_easyA->table('cwl_budongxiao_config')->where('id=1')->find();
        $select_map_sys = $this->db_easyA->table('cwl_budongxiao_history_map_sys')->field('create_time,rand_code')->order('id desc')->find();
        ...
        // dump( $map);die;
        $select_history_sys = $this->db_easyA->table('cwl_budongxiao_history_sys')->field('商品负责人,中类')->group('商品负责人,中类')->limit(100000)->select()->toArray();

        // echo '<pre>';
        // print_r($select_history_sys);
        // die;
        foreach ($select_history_sys as $key => $val) {
            $map_str = "上市天数>='{$select_config['上市天数']}' AND 店铺库存数量>1 AND 商品负责人='{$val['商品负责人']}' AND 季节归集='{$select_config['季节归集']}' AND 中类='{$val['中类']}'";
            echo $sql = "
                SELECT
                    count(*) AS 相同中类数 
                FROM
