...
/**
 * @ControllerAnnotation(title="不动销_冬天")
 */
class AutosendWinter extends BaseController
{
    // 接收筛选参数
    public $params = [];
    // 数据库
    ...
    public function sysReshandle() {
        ...
        // 删除所有基础计算结果
        $this->db_easyA->table('cwl_budongxiao_result_sys_winter')->where(1)->delete();
        // 删除所有详情结果
        $this->db_easyA->table('cwl_budongxiao_history_sys_winter')->where(1)->delete();
        $this->db_bi->table('ww_budongxiao_history_sys_winter')->where(1)->delete();
        // 删除所有统计结果
        CwlBudongxiaoStatisticsSysWinter::delStatic();
        ...
        if ($data) {
            // 统计详情
            $res1 = $this->db_easyA->table('cwl_budongxiao_history_map_sys_winter')->insert([
            ...
            foreach($chunk_list as $key => $val) {
                // 基础结果 
                $this->db_easyA->table('cwl_budongxiao_result_sys_winter')->strict(false)->insertAll($val);
                $this->db_bi->table('ww_budongxiao_result_winter')->strict(false)->insertAll($val);
            }
            echo 1;
    ...
    public function areaNumHandle() {
        $select_config = $this->db_easyA->table('cwl_budongxiao_config')->where('id=2')->find();
        $select_map_sys = $this->db_easyA->table('cwl_budongxiao_history_map_sys_winter')->field('create_time,rand_code')->order('id desc')->find();
        ...
        // $map[] = ['店铺库存数量', '>', '1'];
        // dump( $map);die;
        $select_history_sys = $this->db_easyA->table('cwl_budongxiao_history_sys_winter')->field('省份,中类')->where([
    ...
    public function store($store) {
        ...
        if ($res_all_new) {
            ...
            $insert_history2 = $this->db_bi->table('ww_budongxiao_history_sys_winter')->insertAll($insert_history_data);
            // if ($insert_history) {
            //     $this->db_easyA->commit();
            // } else {
            //     $this->db_easyA->rollback();
