...
class Tableupdate extends BaseController
{
    ...
    public function bi_dianpuyejihuanbi_data()
    {
        ...
        $sql = "
            ...
            WHERE
                ...
                AND EC.ShutOut = 0
                AND EC.RegionId NOT IN ('40', '84', '85',  '97')
    ...
    public function tableTime() {
        $sql = "
            select id,编号,跑数表,跑数表字段 from dd_baobiao where 跑数表 is not null and 数据库 = '新BI' AND 跑数表字段 is not null
        ";
    ...
    public function sp_ww_zt() {
        ...
        $select = $this->db_sqlsrv->query($sql);
        if ($select) {
            $this->db_bi->execute('TRUNCATE sp_ww_zt;');
