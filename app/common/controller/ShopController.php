...
use app\common\constants\ShopContstant;
...
class ShopController extends BaseController
{
    ...
    private function viewInit(){
        ...
        $isSuperAdmin = session('shop.id') == ShopContstant::SUPER_ADMIN_ID ? true : false;
        $data = [
    ...
    private function checkAuth(){
        ...
        $expireTime = session('shop.expire_time');
        /** @var AuthService $authService */
        $authService = app(AuthService::class, ['shopId' => $shopId]);
        ...
        if (!in_array($currentController, $adminConfig['no_login_controller']) &&
            ...
            empty($shopId) && $this->error('请先登录后台', [], __url('shop/login/index'));

            // 判断是否登录过期
            if ($expireTime !== true && time() > $expireTime) {
                session('shop', null);
                $this->error('登录已过期，请重新登录', [], __url('shop/login/index'));
            }

            // 检查当前账户是否合法
            $isLegal = (ShopContstant::AUTH_USER_MODEL)::find($shopId);
            if (!$isLegal) {
                session('shop', null);
                $this->error('身份已过期,请重新登录', [], __url('shop/login/index'));
            }
        }
