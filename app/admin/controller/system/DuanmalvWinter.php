...
/**
 * Class DuanmalvWinter
 * @package app\admin\controller\system
 * @ControllerAnnotation(title="断码率-冬季")
 */
class DuanmalvWinter extends AdminController
{
    // 接收筛选参数
    public $params = [];
    // 数据库
    ...
    public function getConfigMapData() {
        ...
        $select_config = $this->db_easyA->table('cwl_duanmalv_config')->field('不考核门店,不考核货号,季节归集')->where('id=2')->find();
    ...
    public function saveMap() {
        if (request()->isAjax() && checkAdmin()) {
            ...
            // dump($params);die;
            $this->db_easyA->table('cwl_duanmalv_config')->where('id=2')->strict(false)->update($params);     
    ...
    public function zhouxiao() {
        if (request()->isAjax()) {
            ...
            $sql = "
                SELECT 
                    * 
                FROM cwl_duanmalv_retail_winter 
                WHERE 
                    1
                ORDER BY 
    ...
    public function handle() {
        if (request()->isAjax()) {
            ...
            $sql = "
                SELECT 
                    *
                FROM cwl_duanmalv_handle_1_winter WHERE 1
                LIMIT {$pageParams1}, {$pageParams2}  
            ";
            $select = $this->db_easyA->query($sql);
            ...
            $sql2 = "
                SELECT 
                    count(*) as total
                FROM cwl_duanmalv_handle_1_winter
                WHERE 
                    1
            ";
            $count = $this->db_easyA->query($sql2);
    ...
    public function table6() {
        if (request()->isAjax()) {
            $find_config = $this->db_easyA->table('cwl_duanmalv_config')->where('id=2')->find();
            // 筛选条件
            $input = input();
            $pageParams1 = ($input['page'] - 1) * $input['limit'];
            $pageParams2 = input('limit');
            ...
            $sql = "
                ...
                FROM cwl_duanmalv_table6_winter WHERE 1
                    {$map1}
                    {$map2}
                    {$map3}
                    {$map4}
            ...
            $sql2 = "
                SELECT 
                    count(*) as total
                FROM cwl_duanmalv_table6_winter
                WHERE 
                    1
                    {$map1}
                    {$map2}
    ...
    public function table5() {
        if (request()->isAjax()) {
            ...
            $sql = "
                SELECT 
                    *
                FROM cwl_duanmalv_table5_winter WHERE 1
                LIMIT {$pageParams1}, {$pageParams2}  
            ";
            $select = $this->db_easyA->query($sql);
            ...
            $sql2 = "
                SELECT 
                    count(*) as total
                FROM cwl_duanmalv_table5_winter
                WHERE 
                    1
            ";
            $count = $this->db_easyA->query($sql2);
    ...
    public function table4() {
        if (request()->isAjax()) {
            ...
            $sql = "
                ...
                FROM
                    cwl_duanmalv_table4_winter AS t4
                    left join cwl_duanmalv_table4_winter as yn on t4.货号=yn.货号 and yn.省份='云南省'
                    left join cwl_duanmalv_table4_winter as sc on t4.货号=sc.货号 and sc.省份='四川省'
                    left join cwl_duanmalv_table4_winter as tj on t4.货号=tj.货号 and tj.省份='天津'
                    left join cwl_duanmalv_table4_winter as nx on t4.货号=nx.货号 and nx.省份='宁夏回族自治区'
                    left join cwl_duanmalv_table4_winter as ah on t4.货号=ah.货号 and ah.省份='安徽省'
                    left join cwl_duanmalv_table4_winter as gd on t4.货号=gd.货号 and gd.省份='广东省'
                    left join cwl_duanmalv_table4_winter as gx on t4.货号=gx.货号 and gx.省份='广西壮族自治区'
                    left join cwl_duanmalv_table4_winter as xj on t4.货号=xj.货号 and xj.省份='新疆维吾尔自治区'
                    left join cwl_duanmalv_table4_winter as jx on t4.货号=jx.货号 and jx.省份='江西省'
                    left join cwl_duanmalv_table4_winter as henan on t4.货号=henan.货号 and henan.省份='河南省'
                    left join cwl_duanmalv_table4_winter as zj on t4.货号=zj.货号 and zj.省份='浙江省'
                    left join cwl_duanmalv_table4_winter as hainan on t4.货号=hainan.货号 and hainan.省份='海南省'
                    left join cwl_duanmalv_table4_winter as hb on t4.货号=hb.货号 and hb.省份='湖北省'
                    left join cwl_duanmalv_table4_winter as hunan on t4.货号=hunan.货号 and hunan.省份='湖南省'
                    left join cwl_duanmalv_table4_winter as gs on t4.货号=gs.货号 and gs.省份='甘肃省'
                    left join cwl_duanmalv_table4_winter as fj on t4.货号=fj.货号 and fj.省份='福建省'
                    left join cwl_duanmalv_table4_winter as gz on t4.货号=gz.货号 and gz.省份='贵州省'
                    left join cwl_duanmalv_table4_winter as cq on t4.货号=cq.货号 and cq.省份='重庆'
                    left join cwl_duanmalv_table4_winter as xx on t4.货号=xx.货号 and xx.省份='陕西省'
                    left join cwl_duanmalv_table4_winter as qh on t4.货号=qh.货号 and qh.省份='青海省'
                --  where sk.省份='浙江省'
                --  and sk.货号='B32502003'
                WHERE 1
                    {$map6}
            ...
            $sql2 = "
                select count(*) as total from(
                   SELECT 
                    count(*) as total
                    FROM cwl_duanmalv_table4_winter AS t4
                    WHERE 1
                        {$map6}
                        {$map7}
                        {$map8}
        ...
        if (request()->isAjax()) {
            ...
            $sql = "
                ...
                FROM
                    ...
                    left join cwl_duanmalv_table1_1_winter t2 ON t1.店铺名称=t2.店铺名称 and t2.更新日期 = '{$limitDate["newDate"]}'
                    left join cwl_duanmalv_table1_1_winter t3 ON t1.店铺名称=t3.店铺名称 and t3.更新日期 = '{$limitDate["oldDate"]}'
                WHERE
                    {$map0}
                    {$map1}
                    {$map2}
    ...
    public function table1_month() {
        ...
        if (request()->isAjax()) {
            ...
            if (count($select_date) == 9) {
                $count_more_join = "left join cwl_duanmalv_table1_1_winter t9 ON t0.店铺名称=t9.店铺名称 and t9.更新日期 = '{$select_date[8]["更新日期"]}'";
                $count_more_field = "
                ...
                $count_more_join = "
                    left join cwl_duanmalv_table1_1_winter t9 ON t0.店铺名称=t9.店铺名称 and t9.更新日期 = '{$select_date[8]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t10 ON t0.店铺名称=t10.店铺名称 and t10.更新日期 = '{$select_date[9]["更新日期"]}'
                ";
                $count_more_field = "
                    , concat(round(t9.`齐码率-整体` * 100, 1), '%') as `齐码率-整体-t9`
                    , concat(round(t9.`齐码率-TOP实际` * 100, 1), '%') as `齐码率-TOP实际-t9`
            ...
            $sql2 = "
                SELECT  
                    ...
                    {$count_more_field}
                FROM
                    cwl_duanmalv_table1_1_winter t0 
                    left join cwl_duanmalv_table1_1_winter t1 ON t0.店铺名称=t1.店铺名称 and t1.更新日期 = '{$select_date[0]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t2 ON t0.店铺名称=t2.店铺名称 and t2.更新日期 = '{$select_date[1]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t3 ON t0.店铺名称=t3.店铺名称 and t3.更新日期 = '{$select_date[2]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t4 ON t0.店铺名称=t4.店铺名称 and t4.更新日期 = '{$select_date[3]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t5 ON t0.店铺名称=t5.店铺名称 and t5.更新日期 = '{$select_date[4]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t6 ON t0.店铺名称=t6.店铺名称 and t6.更新日期 = '{$select_date[5]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t7 ON t0.店铺名称=t7.店铺名称 and t7.更新日期 = '{$select_date[6]["更新日期"]}'
                    left join cwl_duanmalv_table1_1_winter t8 ON t0.店铺名称=t8.店铺名称 and t8.更新日期 = '{$select_date[7]["更新日期"]}'
                    {$count_more_join}
                WHERE 1
        ...
        // if (1) {
            ...
            $sql = "
                SELECT
                    ...
                    t3.`更新日期` as `更新日期-旧` 
                FROM
                    cwl_duanmalv_table1_2_winter t1 
                    left join cwl_duanmalv_table1_2_winter t2 ON t1.云仓=t2.云仓 and t1.商品负责人=t2.商品负责人 and t2.更新日期 = '{$limitDate["newDate"]}'
                    left join cwl_duanmalv_table1_2_winter t3 ON t1.云仓=t3.云仓 and t1.商品负责人=t3.商品负责人 and t3.更新日期 = '{$limitDate["oldDate"]}'
                WHERE
                    {$map0} 
                GROUP BY
                    t1.云仓, t1.商品负责人
            ...
            $sql3 = "
                SELECT
                    ...
                    t2.*
                    FROM
                    cwl_duanmalv_table1_avg_winter t1,  (				
                        SELECT
                            '' as `齐码排名-旧`,
                            `直营-整体` as `直营-整体-旧`,
                            `加盟-整体` as `加盟-整体-旧`,
                            ...
                            `更新日期` as `更新日期-旧` 
                        FROM
                        cwl_duanmalv_table1_avg_winter  
                        WHERE
        ...
        if (request()->isAjax()) {
            ...
            $sql = "
                SELECT
                    ...
                    t3.`更新日期` as `更新日期-旧` 
                FROM
                    cwl_duanmalv_table1_3_winter t1 
                    left join cwl_duanmalv_table1_3_winter t2 ON t1.省份=t2.省份 and t1.商品负责人=t2.商品负责人 and t2.更新日期 = '{$limitDate["newDate"]}'
                    left join cwl_duanmalv_table1_3_winter t3 ON t1.省份=t3.省份 and t1.商品负责人=t3.商品负责人 and t3.更新日期 = '{$limitDate["oldDate"]}'
                WHERE
                    {$map0}
                GROUP BY
                    t1.省份, t1.商品负责人
    ...
    public function getXmMapSelect() {
        ...
        $customer17 = $this->db_easyA->query("
            SELECT 商品负责人 as name, 商品负责人 as value FROM cwl_duanmalv_sk_winter WHERE 商品负责人 IS NOT NULL GROUP BY 商品负责人
        ");
        // 商品负责人
        if (! checkAdmin()) { 
            // $admin = session('admin.name');
        ...
        $province = $this->db_easyA->query("
            SELECT 省份 as name, 省份 as value FROM cwl_duanmalv_sk_winter WHERE 省份 IS NOT NULL GROUP BY 省份
        ");
        ...
        $zhonglei = $this->db_easyA->query("
            SELECT 二级分类 as name, 二级分类 as value FROM cwl_duanmalv_sk_winter WHERE  二级分类 IS NOT NULL GROUP BY 二级分类
        ");
        $lingxing = $this->db_easyA->query("
            SELECT 领型 as name, 领型 as value FROM cwl_duanmalv_sk_winter WHERE  领型 IS NOT NULL GROUP BY 领型
        ");
        $huohao = $this->db_easyA->query("
            SELECT 货号 as name, 货号 as value FROM cwl_duanmalv_sk_winter WHERE  货号 IS NOT NULL GROUP BY 货号
        ");
