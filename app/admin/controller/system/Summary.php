...
class Summary extends AdminController
{
        ...
        if (request()->isAjax()) {
            ...
            $sql = "
                SELECT
                    ...
                    `单款超量SKC数`,
                    concat(round(销售贡献比_外套 * 100, 1), '%') as `销售贡献比_外套`,
                    concat(round(销售贡献比_内搭 * 100, 1), '%') as `销售贡献比_内搭`,
                    concat(round(销售贡献比_下装 * 100, 1), '%') as `销售贡献比_下装`,
                    concat(round(销售贡献比_鞋履 * 100, 1), '%') as `销售贡献比_鞋履`,
                    concat(round(特价占比 * 100, 1), '%') as 特价占比,
                    周转_外套,周转_内搭,周转_下装,周转_鞋履,售空_外套,售空_内搭,售空_下装,售空_鞋履,不动销需调整SKC_秋,不动销需调整SKC_冬,
                    断码率情况_秋_外套,断码率情况_秋_内搭,断码率情况_秋_下装,断码率情况_秋_鞋履,断码率情况_冬_外套,
                    断码率情况_冬_内搭,断码率情况_冬_下装,断码率情况_冬_鞋履
                FROM
                    cwl_summary 
                WHERE 1	
                    {$map0}
    ...
    public function excel_summary() {
        if (request()->isAjax()) {
            ...
            $map = "{$map0}{$map1}{$map2}{$map3}{$map4}{$map5}{$map6}{$map7}{$map8}{$map9}{$map10}{$map11}";
            $code = rand_code(6);
            cache($code, $map, 3600);
            return json([
                'status' => 1,
            ...
            return Excel::exportData($select, $header, '自动提醒汇总表_' . date('Ymd') . '_' . time() , 'xlsx');
        }
    }  
