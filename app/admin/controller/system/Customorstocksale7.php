...
class Customorstocksale7 extends AdminController
{
    ...
    public function index1()
    {
        if ($this->request->isAjax()) {
            ...
            ->field('省份,气温区域,风格,季节,分类,经营模式,店铺名称,店铺等级,一级分类,二级分类,
            sum(前七天销售数量) as 前七天店均销,
            sum(前六天销售数量) as 前六天店均销,
            sum(前五天销售数量) as 前五天店均销,
            sum(前四天销售数量) as 前四天店均销,
            sum(前三天销售数量) as 前三天店均销,
            sum(前二天销售数量) as 前二天店均销,
            sum(前一天销售数量) as 前一天店均销, 
            sum(((前一天库存数量 + 前二天库存数量 + 前三天库存数量 + 前四天库存数量 + 前五天库存数量 + 前六天库存数量 + 前七天库存数量)/7)) as 近一周店均库存,
            sum(((前七天销售数量 + 前六天销售数量+前五天销售数量+前四天销售数量+前三天销售数量+前二天销售数量+前一天销售数量) / 7)) as 近一周店均销,
            sum(前七天库存数量) as 前七天店均库存,
            sum(前六天库存数量) as 前六天店均库存,
            sum(前五天库存数量) as 前五天店均库存,
            sum(前四天库存数量) as 前四天店均库存,
            sum(前三天库存数量) as 前三天店均库存,
            sum(前二天库存数量) as 前二天店均库存,
            sum(前一天库存数量) as 前一天店均库存, 
            sum(前七天库存成本) as 前七天库存成本,
            sum(前六天库存成本) as 前六天库存成本,
            sum(前五天库存成本) as 前五天库存成本,
            sum(前四天库存成本) as 前四天库存成本,
            sum(前三天库存成本) as 前三天库存成本,
            sum(前二天库存成本) as 前二天库存成本,
            sum(前一天库存成本) as 前一天库存成本,
            sum(前七天成本金额) as 前七天成本金额,
            sum(前六天成本金额) as 前六天成本金额,
            sum(前五天成本金额) as 前五天成本金额,
            sum(前四天成本金额) as 前四天成本金额,
            sum(前三天成本金额) as 前三天成本金额,
            sum(前二天成本金额) as 前二天成本金额,
            sum(前一天成本金额) as 前一天成本金额,
            sum(前七天销售金额) as 前七天销售金额,
            sum(前六天销售金额) as 前六天销售金额,
            sum(前五天销售金额) as 前五天销售金额,
            sum(前四天销售金额) as 前四天销售金额,
            sum(前三天销售金额) as 前三天销售金额,
            sum(前二天销售金额) as 前二天销售金额,
            sum(前一天销售金额) as 前一天销售金额,
            sum(前六天零售金额) as 前六天零售金额,
            sum(前七天零售金额) as 前七天零售金额,
            sum(前五天零售金额) as 前五天零售金额,
            sum(前四天零售金额) as 前四天零售金额,
            sum(前三天零售金额) as 前三天零售金额,
            sum(前二天零售金额) as 前二天零售金额,
            sum(前一天零售金额) as 前一天零售金额,
            sum(ifnull((前七天库存成本/前七天成本金额), 0)) as 前七天金额周转天,
            sum(ifnull((前六天库存成本/前六天成本金额), 0)) as 前六天金额周转天,
            sum(ifnull((前五天库存成本/前五天成本金额), 0)) as 前五天金额周转天,
            sum(ifnull((前四天库存成本/前四天成本金额), 0)) as 前四天金额周转天,
            sum(ifnull((前三天库存成本/前三天成本金额), 0)) as 前三天金额周转天,
            sum(ifnull((前二天库存成本/前二天成本金额), 0)) as 前二天金额周转天, 
            sum(ifnull((前一天库存成本/前一天成本金额), 0)) as 前一天金额周转天,
            sum(ifnull(((前一天库存成本+前二天库存成本+前三天库存成本+前四天库存成本+前五天库存成本+前六天库存成本+前七天库存成本)/(前一天成本金额+前二天成本金额+前三天成本金额+前四天成本金额+前五天成本金额+前六天成本金额+前七天成本金额)), 0) ) as 近一周金额周转天,
            sum(预计库存) as 昨日门店预计库存'
            )
    ...
    public function index2()
    {
        ...
        // if (1) {
            ...
            前二天销售金额,前一天销售金额,前六天零售金额,前七天零售金额,前五天零售金额,前四天零售金额,前三天零售金额,前二天零售金额,前一天零售金额,ifnull((前七天库存成本/前七天成本金额), 0) as 前七天金额周转天,
