 ...
 */
class Customeryejijiameng extends BaseController
{
    protected $db_easyA = '';
    protected $db_bi = '';
    protected $db_sqlsrv = '';
    ...
    public function getData() {
        ...
        if ($select) {
            ...
            $this->db_easyA->table('dd_customer_yeji_data_jm')->where([
                ['销售日期', '=', $date]
            ])->delete();

            $chunk_list = array_chunk($select, 500);
            foreach($chunk_list as $key => $val) {
                $this->db_easyA->table('dd_customer_yeji_data_jm')->strict(false)->insertAll($val);
            }

            $sql_修正 = "
                UPDATE `dd_customer_yeji_data_jm` 
                SET
                    季节修正 = 
                        case
            ...
            $this->db_easyA->table('dd_customer_yeji_jm')->where([
                ['销售日期', '=', $date]
            ])->delete();

            $chunk_list2 = array_chunk($select2, 500);
            foreach($chunk_list2 as $key => $val) {
                $this->db_easyA->table('dd_customer_yeji_jm')->strict(false)->insertAll($val);
            }

            $this->db_easyA->table('dd_customer_yeji_avg_jm')->where([
                ['销售日期', '=', $date]
            ])->delete();
            $sql_更新同区平均值 = "
                SELECT
            ...
            foreach($chunk_list3 as $key => $val) {
                $this->db_easyA->table('dd_customer_yeji_avg_jm')->strict(false)->insertAll($val);
            }
    ...
    public function getCustomer() {
        $date = input('date') ? input('date') : date('Y-m-d');
        $select_customer = $this->db_easyA->table('dd_customer_yeji_jm')->field('店铺名称,温区')->where([
            ['销售日期', '=', $date],
            // ['店铺名称', '=', '南宁二店'],
        ...
        foreach ($select_customer as $k => $v) {
            $sql = "
                SELECT
                    ...
                    avg.数量 as 平均数
                FROM
                    dd_customer_yeji_avg_jm AS avg
                LEFT JOIN 
                    dd_customer_yeji_jm AS c ON  avg.销售日期 = c.销售日期 AND  avg.温区 = c.温区 AND avg.季节修正 = c.季节修正 AND avg.品类 = c.品类 
                    AND c.店铺名称 = '{$v["店铺名称"]}'
                WHERE 1
                    AND avg.店铺名称 = '同区平均'
                    AND avg.温区 = '{$v["温区"]}'
            ...
            $业绩_春秋季 =$this->db_easyA->table('dd_customer_yeji_data_jm')->field('店铺名称,温区')->where([
                ['销售日期', '=', $date],
                ['店铺名称', '=', $v['店铺名称']],
                ['季节修正', '=', '春/秋季'],
            ])->sum('销售金额');

            $业绩_夏季 =$this->db_easyA->table('dd_customer_yeji_data_jm')->field('店铺名称,温区')->where([
                ['销售日期', '=', $date],
                ['店铺名称', '=', $v['店铺名称']],
                ['季节修正', '=', '夏季'],
            ])->sum('销售金额');

            $业绩_冬季 =$this->db_easyA->table('dd_customer_yeji_data_jm')->field('店铺名称,温区')->where([
                ['销售日期', '=', $date],
                ['店铺名称', '=', $v['店铺名称']],
                ['季节修正', '=', '冬季'],
            ])->sum('销售金额');
            ...
            if ($data) {
                ...
                $params = [
                    ...
                    'banben' => '',
                    'file_path' => app()->getRootPath() . 'public/upload/dd_customer_yeji_jm/' . date('Ymd', time()) . '/'  //文件保存路径
                ];
    ...
    public function sendDingImg() {
        ...
        foreach ($select as $key => $val) {
            // 线上
            $path = "http://im.babiboy.com/upload/dd_customer_yeji_jm/{$datatime}/{$val['店铺名称']}.jpg";
