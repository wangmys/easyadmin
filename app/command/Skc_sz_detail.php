...
class Skc_sz_detail extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $all_customers = Db::connect("mysql2")->Query("select c.*,cr.首单日期 from sp_ww_customer c inner join customer_regionid cr on c.店铺名称=cr.店铺名称 where c.经营模式 in ('直营', '加盟') and cr.RegionId not in ($customer_regionid_notin);");
