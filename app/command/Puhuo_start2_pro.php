...
class Puhuo_start2_pro extends Command
{
    protected $db_easy;
    ...
    {
        // 指令配置
        $this->setName('Puhuo_start2_pro')
            ->addArgument('list_rows', Argument::OPTIONAL)//每页条数
            ->setDescription('the Puhuo_start2_pro command');
                                ...
                                if ($add_all_arr) {
                                    ...
                                    try {
                                        ...
                                        foreach ($add_all_arr as $v_customer) {
                                            ...
                                            //记录铺货日志：
                                            Log::channel('puhuo')->write('##############套装套西-云仓-货号-店铺:'.$WarehouseName.$GoodsNo.$v_customer['CustomerName'].'##############'.json_encode(['uuid'=>$uuid, 'GoodsNo'=>$GoodsNo, 'CustomerName'=>$v_customer['CustomerName'],  'rule'=>$rule, 'v_data'=>$v_data, 'puhuo_config'=>$puhuo_config, 'goods_yuji_stock'=>$goods_yuji_stock, 'current_14days'=>$current_14days, 'can_puhuo结果：'=>$can_puhuo]) );
